...
# Example 13 Get the script libraryOld path
def getScriptLibraryPath():
        ...
        # get the script libraryOld paths
        if (scriptLibFiles != ""):
           for file in scriptLibFiles:
...
# Example 14 Get the script libraryOld files #
def getScriptLibraryFiles():
        libPaths = []
        fs = java.lang.System.getProperty("file.separator")

        # Find the wsadmin script libraryOld path
        ...
        # Find the custom scipt libraryOld path
...
def fileSearch(directory, paths):
    if (directory != ""):
       ...
       # Recrusive search script libraryOld files in the directory
       for file in files:
           if (file.isDirectory()):
