...
Introduction
------------
wsadminlib is a libraryOld to simplify configuring WebSphere.
...
def findCoreGroupIdForServer(servername):
    ...
    if coregroup_id == None:
        # raise m + " Error: Could not find coregroup. coregroupname=%s" % (coregroupname)
        raise m + " Error: Could not find coregroup. coregroupname=%s" % (cg_name)
...
def setCustomPropertyOnObject(object_id, propname, propvalue):
    ...
    if value != None:
        ...
        raise Exception("Could not find property %s in server even though it seemed to be there earlier" % (propname))
    else:
...
def createSharedLibrary(libname, jarfile):
    """Creates a shared libraryOld on the specified cell with the given name and jarfile"""
    m = "createSharedLibrary:"
    # sop(m,"Entry. Create shared libraryOld. libname=%s jarfile=%s" % (repr(libname), repr(jarfile) ))
    cellname = getCellName();
...
def createSharedLibraryIsolated (libname, jarfile, isolated):
    """Creates an isolated shared libraryOld on the specified cell with the given name and jarfile"""
    m = "createSharedLibraryIsolated:"
    # sop(m,"Entry. Create shared libraryOld isolated. libname=%s jarfile=%s isolated=%s" % (repr(libname), repr(jarfile), repr(isolated) ))
    cellname = getCellName();
    cell_id = getCellId(cellname);
    # sop(m,"cell_id=%s " % ( repr(cell_id), ))
...
def associateSharedLibrary (libname, appname, warname):
    """Associates the installed shared libraryOld with an EAR"""
    m = "associateSharedLibrary:"
    # sop(m,"Entry. Associate the shared libraryOld with an EAR. libname=%s appname=%s warname=%s " % ( repr(libname), repr(appname), repr(warname) ))
...
def deleteSharedLibrary(libname):
    """Delete shared libraryOld with specified name"""
...
def createSharedLibraryClassloader(nodename, servername, libname):
    """Creates a classloader on the specified appserver and associates it with a shared libraryOld"""
    m = "createSharedLibraryClassloader:"
    # sop(m,"Entry. Create shared libraryOld classloader. nodename=%s servername=%s libname=%s " % ( repr(nodename), repr(servername), repr(libname) ))
    server_id = getServerByNodeAndName(nodename, servername)
    # sop(m,"server_id=%s " % ( repr(server_id), ))
